...
/*  @PostMapping("/register")
  ...
  public Result checkMobile(@RequestBody CustomerRegisterRequest customerRegisterRequest){
    String mobile = customerRegisterRequest.getMobile();
    if(null == mobile || mobile.length()==0) {
    	return Result.ok("手机号码不能为空");
    }
    int count = wxCustomerQueryMapper.checkMobile(mobile);
    if(count > 0 ) {
    	return Result.ok("验证成功");
    }else {
    	return Result.fail("验证失败");
